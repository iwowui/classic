...
elseif GetLocale() == "zhTW" then
	ItemRack.CheckButtonLabels = {
		["ItemRackOptItemStatsPriorityText"] = "優先順序",
...
function ItemRackOpt.OnLoad(self)
	...
	if GetLocale() == "zhCN" then
		ItemRackOpt.OptInfo = {
			...
			{type="check",optset=ItemRackUser,variable="EnablePerSetQueues",depend="EnableQueues",label="启用预设队列",tooltip="Enable individual auto queues per set."},
			...
			{type="check",optset=ItemRackSettings,variable="ShowSetInTooltip",label="显示套装鼠标提示",tooltip="Show which set an item belongs to in the tooltip."},
	...
	elseif GetLocale() == "zhTW" then
		ItemRackOpt.OptInfo = {
			{type="label",label=(UnitName("player")).."的設置"},
			{type="check",optset=ItemRackUser,variable="Locked",label="鎖定按鈕",tooltip="Prevent buttons and menus from being moved."},
			{type="check",optset=ItemRackUser,variable="EnableEvents",label="啟用事件",tooltip="Enable events to automatically swap gear."},
			{type="check",optset=ItemRackUser,variable="EnableQueues",label="啟用自動佇列",tooltip="Enables auto queues to automatically swap gear."},
			{type="check",optset=ItemRackUser,variable="EnablePerSetQueues",depend="EnableQueues",label="啟用預設佇列",tooltip="Enable individual auto queues per set."},
			{type="number",optset=ItemRackUser,variable="ButtonSpacing",button=ItemRackOptButtonSpacing,label="按鈕間距",tooltip="Padding distance between buttons.",combatlock=1},
			{type="slider",button=ItemRackOptButtonSpacingSlider,variable="ButtonSpacing",label="按鈕間距",tooltip="Padding distance between buttons.", min=0, max=24, step=1, form="%d",combatlock=1},
			{type="number",optset=ItemRackUser,variable="Alpha",button=ItemRackOptAlpha,label="透明度",tooltip="Transparency (alpha) of the buttons and menu."},
			{type="slider",button=ItemRackOptAlphaSlider,variable="Alpha",label="透明度",tooltip="Transparency (alpha) of the buttons and menu.", min=.1, max=1, step=.05, form="%.2f"},
			{type="number",optset=ItemRackUser,variable="MainScale",button=ItemRackOptMainScale,label="按鈕大小",tooltip="Scale size of the item buttons.",combatlock=1},
			{type="slider",button=ItemRackOptMainScaleSlider,variable="MainScale",label="按鈕大小",tooltip="Scale size of the item buttons.", min=.5, max=2, step=.05, form="%.2f",combatlock=1},
			{type="number",optset=ItemRackUser,variable="MenuScale",button=ItemRackOptMenuScale,label="菜單大小",tooltip="Scale size of the menu in relation to the button it's docked to."},
			{type="slider",button=ItemRackOptMenuScaleSlider,variable="MenuScale",label="菜單大小",tooltip="Scale size of the menu.", min=.5, max=2, step=.05, form="%.2f"},
			{type="check",optset=ItemRackUser,variable="SetMenuWrap",label="裝備菜單換行",tooltip="Check this to set a fixed value when the menu wraps to a new row.  Uncheck to let ItemRack decide."},
			{type="number",optset=ItemRackUser,variable="SetMenuWrapValue",depend="SetMenuWrap",button=ItemRackOptSetMenuWrapValue,label="何時換行",tooltip="When 'Set menu wrap' checked, this is the number of menu items before wrapping to a new row/column."},
			{type="slider",optset=ItemRackUser,button=ItemRackOptSetMenuWrapValueSlider,depend="SetMenuWrap",variable="SetMenuWrapValue",label="何時換行",tooltip="When 'Set menu wrap' checked, this is the number of menu items before wrapping to a new row/column.", min=1, max=30, step=1, form="%d"},
			{type="label",label="全域設置"},
			{type="check",optset=ItemRackSettings,variable="MenuOnShift",label="轉換菜單",tooltip="Only show menu while Shift is held down."},
			{type="check",optset=ItemRackSettings,variable="MenuOnRight",label="右鍵菜單",tooltip="Open menu by right clicking buttons.",combatlock=1},
			{type="check",optset=ItemRackSettings,variable="HideOOC",label="脫戰隱藏",tooltip="Hide the buttons while out of combat.",combatlock=1},
			{type="check",optset=ItemRackSettings,variable="HidePetBattle",label="在寵物戰鬥中隱藏",tooltip="Hide the buttons during a pet battle."},
			{type="check",optset=ItemRackSettings,variable="Notify",label="就緒時通告",tooltip="Announce when an item you used comes off cooldown."},
			{type="check",optset=ItemRackSettings,variable="NotifyThirty",label="30秒時通告",tooltip="Announce when an item you used is at 30 seconds cooldown."},
			{type="check",optset=ItemRackSettings,variable="NotifyChatAlso",label="通告在聊天框",tooltip="Send cooldown notifications to chat also."},
			{type="check",optset=ItemRackSettings,variable="ShowSetInTooltip",label="顯示套装滑鼠提示",tooltip="Show which set an item belongs to in the tooltip."},
			{type="check",optset=ItemRackSettings,variable="ShowTooltips",label="顯示滑鼠提示",tooltip="Show tooltips like the one you're reading now."},
			{type="check",optset=ItemRackSettings,variable="TinyTooltips",depend="ShowTooltips",label="迷你滑鼠提示",tooltip="Shrink item tooltips to display only name, cooldown and durability."},
			{type="check",optset=ItemRackSettings,variable="TooltipFollow",depend="ShowTooltips",label="滑鼠提示跟隨",tooltip="Show tooltips near the mouse."},
			{type="check",optset=ItemRackSettings,variable="CooldownCount",label="冷卻數字",tooltip="Display the cooldown time as a number over items."},
			{type="check",optset=ItemRackSettings,variable="LargeNumbers",depend="CooldownCount",label="大數字",tooltip="Use a larger font for cooldown numbers."},
			{type="check",optset=ItemRackSettings,variable="Cooldown90",depend="CooldownCount",label="90秒CD倒計時",tooltip="Use seconds instead of minutes starting at 90 seconds remaining."},
			{type="check",optset=ItemRackSettings,variable="AllowEmpty",label="允許空槽",tooltip="Add an empty slot to menus of equipped items."},
			{type="check",optset=ItemRackSettings,variable="AllowHidden",label="允許隱藏裝備",tooltip="Enable Alt+clicking of menu items to hide/show them in the menu.  Hold Alt as you enter a menu to show all."},
			{type="check",optset=ItemRackSettings,variable="HideTradables",label="隱藏可交易物",tooltip="Prevent tradable items from showing up in the menu."},
			{type="check",optset=ItemRackSettings,variable="ShowMinimap",label="顯示小地圖按鈕",tooltip="Show the minimap button to access options or change sets."},
			{type="check",optset=ItemRackSettings,variable="SquareMinimap",depend="ShowMinimap",label="方形小地圖",tooltip="If you use a square minimap, make the button drag along square edge."},
			{type="check",optset=ItemRackSettings,variable="MinimapTooltip",depend="ShowMinimap",label="顯示小地圖滑鼠提示",tooltip="If tooltips enabled, show what mouse clicks will do when clicking the minimap button."},
			{type="check",optset=ItemRackSettings,variable="TrinketMenuMode",label="TrinketMenu(飾品)模式",tooltip="When mouseover of either trinket slot, open anchored to the top trinket.  Left click of a menu item will equip to the top trinket.  Right click will equip to the bottom trinket."},
			{type="check",optset=ItemRackSettings,variable="AnchorOther",depend="TrinketMenuMode",label="錨定其他飾品",tooltip="In TrinketMenu mode, trinket menus dock to the top trinket.  Check this to anchor them to the bottom trinket."},
			{type="check",optset=ItemRackSettings,variable="EquipToggle",label="裝備集切換",tooltip="When a set is equipped, if it's already equipped, unequip it."},
			{type="check",optset=ItemRackSettings,variable="ShowHotKeys",label="顯示快捷綁鍵",tooltip="Display key bindings on buttons"},
			{type="check",optset=ItemRackSettings,variable="EquipOnSetPick",label="選項中就裝備",tooltip="Check this to equip sets and items when selecting items in options or from the dropdown in the Sets tab."},
			{type="check",optset=ItemRackSettings,variable="CharacterSheetMenus",label="C鍵時可查看裝備集",tooltip="While this is checked, mouseover of slots on the character sheet will pop out a menu of items that can go in that slot."},
			{type="check",optset=ItemRackSettings,variable="DisableAltClick",label="禁用Alt點擊",tooltip="Alt+Click on buttons dragged from the character sheet toggles auto queue for that slot.  Check this to disable that behavior. (ie to use Alt+click to self cast instead.)",combatlock=1},
			{type="label",label=""},
			{type="button",button=ItemRackOptKeyBindings,label="槽鍵綁定",tooltip="Set key bindings to use slots.",combatlock=1},
			{type="button",button=ItemRackOptResetBar,label="重新開機按鈕",tooltip="Remove all buttons and restore to default alpha and scale.",combatlock=1},
			{type="button",button=ItemRackOptResetEvents,label="重置事件",tooltip="Restore default events or wipe all events to default settings."},
			{type="button",button=ItemRackOptResetEverything,label="重置所有",tooltip="Wipe all settings, sets and events to restore mod to a default state.",combatlock=1},
		}
	end
