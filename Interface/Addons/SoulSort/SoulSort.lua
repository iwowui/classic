...
f:SetScript("OnEvent", function(self, event, arg1, arg2, ...)
	if event == "ADDON_LOADED" and arg1 == addonname then
		...
		if not SoulSortOptions then
			...
			SoulSortOptions.ShowCounterPerBag = false
		end
		SS_InitSoulShardCounter()
	elseif event == "PLAYER_REGEN_ENABLED" and SoulSortOptions.AutoSort == true then
		SS_SortShards()
...
function SS_DeleteExcessShards()
	if SoulSortOptions.MaxShards == 0 then return end
