...
local function _InitMenu(self)
	-- sort cmds
	local cmds = {}
	self.info = Lib_UIDropDownMenu_CreateInfo()
	...
	-- create menu
 	self.frame = CreateFrame("Frame", self.frameName, UIParent, "Lib_UIDropDownMenuTemplate")
 	self.frame.obj = self
 	self.cmds = cmds
	Lib_UIDropDownMenu_Initialize(self.frame, _DropDown_Initialize, "MENU", 1, self.cmds)
end
...
function vendor.PopupMenu.prototype:Show()
	Lib_ToggleDropDownMenu(nil, nil, self.frame, "cursor", -20, 0, self.cmds)
end
