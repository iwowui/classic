...
LibEvent:attachTrigger("INSPECT_FRAME_COMPARE", function(self, frame)
    ...
    if (MerInspectDB and MerInspectDB.ShowOwnFrameWhenInspecting) then
        ...
        if (frame.statsFrame2) then
            frame.statsFrame2:SetParent(playerFrame)
        end
    else
        if (frame.statsFrame) then
        ...
        if (frame.statsFrame2) then
            frame.statsFrame2:SetParent(frame)
        end
    end
    if (frame.statsFrame) then
        frame.statsFrame:ClearAllPoints()
    ...
    if (frame.statsFrame2) then
        ...
        frame.statsFrame2:ClearAllPoints()
        frame.statsFrame2:SetPoint("TOPLEFT", frame:GetParent(), "TOPRIGHT", -x, -y)
    end
end)
