...
function RecipeRadar_RecipesTab_OnClick(self)
   ...
   if (RecipeRadar.db.profile.ActiveTab == 2) then
      return
   end
   -- IG_SPELLBOOK_CLOSE
   PlaySound(830)
...
function RecipeRadar_RecipesTab_Update()
   ...
   local prof = RecipeRadar.db.profile.CurrentProfession

   TitleBarText:SetText(L["Recipe Radar Classic"] .. " - " .. prof .. " " ..
         format(PARENS_TEMPLATE, RecipeRadar_SkillDB_GetRank(prof)))
   ...
   for _, location in pairs(recipe.Locations) do
      ...
      if (location.IsFiltered) then
         ...
         if (index > 0 and index <= RECIPERADAR_VENDORS_DISPLAYED) then
            ...
            content = format(PARENS_TEMPLATE, content)
